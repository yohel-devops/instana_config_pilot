...
export function validateConfig({ tags, activeSensors, catalog = {} }) {
  const issues = [];
  const getTag = key => tags.find(t => t.key === key)?.value || '';
...
export function validateUploadedYaml(text) {
  const issues = [];
